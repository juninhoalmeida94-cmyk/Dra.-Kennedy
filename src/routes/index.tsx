...
import { company, whatsappUrl } from "@/client/company";
import { images } from "@/client/images";
import { seo } from "@/client/seo";
import { themeCssVariables } from "@/config/theme";
import { About, CTA, FAQ, Footer, Gallery, Hero, Services, Testimonials } from "@/components/sections";

const resultSlides = [
  {
    kind: "video",
    media: images.videos.result1,
    procedure: company.gallery.slides[0][1],
    angle: company.gallery.slides[0][2],
    objective: company.gallery.slides[0][3],
  },
  {
    kind: "video",
    media: images.videos.result2,
    procedure: company.gallery.slides[1][1],
    angle: company.gallery.slides[1][2],
    objective: company.gallery.slides[1][3],
  },
  {
    kind: "video",
    media: images.videos.result3,
    procedure: company.gallery.slides[2][1],
    angle: company.gallery.slides[2][2],
    objective: company.gallery.slides[2][3],
  },
  {
    kind: "image",
    media: images.gallery.result1,
    procedure: company.gallery.slides[3][1],
    angle: company.gallery.slides[3][2],
    objective: company.gallery.slides[3][3],
  },
  {
    kind: "video",
    media: images.videos.result1,
    procedure: company.gallery.slides[4][1],
    angle: company.gallery.slides[4][2],
    objective: company.gallery.slides[4][3],
  },
  {
    kind: "video",
    media: images.videos.result2,
    procedure: company.gallery.slides[5][1],
    angle: company.gallery.slides[5][2],
    objective: company.gallery.slides[5][3],
  },
];
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { property: "og:image", content: images.hero.main },
      { name: "twitter:card", content: "summary_large_image" },
      { name: "twitter:image", content: images.hero.main },
    ],
    links: [
      { rel: "canonical", href: "/" },
    ],
    scripts: [
      {
        ...
        children: JSON.stringify({
          ...
          telephone: company.contact.whatsappDisplay,
          address: {
            "@type": "PostalAddress",
          ...
          areaServed: company.locationLabel,
        }),
      },
    ],
  }),
...
const WA_DISPLAY = company.contact.whatsappDisplay;
const IG_URL = company.contact.instagramUrl;
const WA = `https://wa.me/${company.contact.whatsappNumber}`;
const waLink = whatsappUrl;

const facial = company.services.categories.principal.items;
const pele = company.services.categories.complementary.items;
const corporal = company.services.categories.additional.items;

function ServiceList({ items }: { items: ReadonlyArray<readonly [string, string]> }) {
  return (
    ...
    <>
      ...
      <div id="curtain" className={curtainHide ? "hide" : ""}>
        <div className="curtain-inner">
          <img src={images.logo} alt={company.name} className="curtain-logo" />
          <p>"{company.tagline}"</p>
        </div>
      </div>
      ...
      <header id="site-header" className={scrolled ? "scrolled" : ""}>
        <nav className="wrap">
          <a href="#hero" className="logo" aria-label={company.name}>
            <img src={images.logo} alt={company.name} className="logo-mark" />
          </a>
      ...
      <Hero>
        <div className="hero-inner">
          <div className="hero-copy">
            <span className="eyebrow reveal">{company.hero.eyebrow}</span>
            <h1 className="reveal delay-1">
              {company.hero.title}
            </h1>
            <p className="hero-sub reveal delay-2">
              {company.hero.description}<br />
              <span className="result-kicker">{company.hero.highlight}</span>
            </p>
            <div className="hero-actions reveal delay-3">
              <a href={waLink(company.messages.schedule)} className="btn btn-solid gold">{company.hero.primaryAction}</a>
              <a href="#procedimentos" className="btn btn-outline">{company.hero.secondaryAction}</a>
            </div>
          </div>
          <div className="hero-photo reveal delay-2">
            <img src={images.hero.main} alt={company.hero.photoAlt} loading="eager" />
            <span className="hero-photo-tag">{company.professionalName}</span>
          </div>
        </div>
        ...
        <div className="wrap">
          ...
          <div className="authority-grid reveal">
            {[
              ...
              ["VI", company.locationLabel, "Empresa de fácil acesso, estrutura acolhedora e discreta."],
            ].map(([n, h, p]) => (
      ...
      <About>
        <div className="wrap about-grid">
          <div className="about-photo reveal">
            <img src={images.hero.about} alt={company.hero.photoAlt} loading="lazy" />
          </div>
      ...
      <Services>
        <div className="wrap">
          ...
          <div className="reveal">
            {tab === "facial" && <ServiceList items={facial} />}
            {tab === "pele" && <ServiceList items={pele} />}
            {tab === "corporal" && <ServiceList items={corporal} />}
          </div>
        </div>
      </Services>
      ...
      <section id="diferenciais">
        <div className="wrap">
          <div className="section-head center reveal">
            <span className="eyebrow">Por que escolher nossa empresa</span>
        ...
        <div className="wrap clinic-grid">
          ...
          <div className="clinic-features reveal delay-1">
            {[
              ...
              ["IV", "Localização acessível", `Estrutura em região central de ${company.locationLabel}.`],
            ].map(([n, h, p]) => (
      ...
      <section id="mentorias">
        <div className="wrap mentorship-shell reveal">
          <div className="mentorship-copy">
            ...
            <p>Além dos atendimentos, nossa equipe compartilha experiência por meio de cursos e mentorias voltados a profissionais que desejam aperfeiçoar suas técnicas, ampliar sua segurança e elevar o nível dos seus resultados.</p>
          </div>
          ...
          <a href={waLink("Olá! Gostaria de receber informações sobre os cursos e mentorias.")} className="btn btn-outline on-light">Conhecer cursos e mentorias</a>
        </div>
      </section>
        ...
        <div className="wrap">
          ...
          <div className="content-grid reveal">
            {[
              ...
              { icon: PlayCircle, title: "Vídeos da empresa", text: "Explicações profissionais sobre nossos serviços e soluções." },
        ...
        <div className="wrap">
          ...
          <ol className="service-steps reveal">
            {[
              ...
              ["02", "Avaliação individual", "Nossa equipe analisa suas necessidades, objetivos e expectativas."],
        ...
        <div className="wrap">
          ...
          <div className="contato-grid reveal">
            <a className="contato-card" href={waLink("Olá, gostaria de agendar uma avaliação.")} target="_blank" rel="noreferrer">
              <img src={images.icons.whatsapp} alt="WhatsApp" />
              <div>
            ...
            <a className="contato-card" href={IG_URL} target="_blank" rel="noreferrer">
              ...
              <div>
                ...
                <strong>{company.contact.instagramHandle}</strong>
      ...
      <section id="localizacao">
        <div className="wrap location-grid">
          <div className="location-text reveal">
            ...
            <h2>Onde estamos</h2>
            <address>{company.address.street}<br />{company.address.city} – {company.address.state}, {company.address.postalCode}</address>
            <div className="location-actions">
              <a className="btn btn-outline on-light" target="_blank" rel="noreferrer" href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(company.address.mapsQuery)}`}>Ver no Google Maps</a>
              <a className="btn btn-outline on-light" target="_blank" rel="noreferrer" href={`https://waze.com/ul?q=${encodeURIComponent(company.address.mapsQuery)}`}>Traçar rota pelo Waze</a>
            </div>
          </div>
          <div className="map-frame reveal delay-1">
            <iframe title="Mapa" src={`https://www.google.com/maps?q=${encodeURIComponent(company.address.mapsQuery)}&output=embed`} width="100%" height="100%" style={{ border: 0, filter: "grayscale(1) contrast(1.05)" }} loading="lazy"></iframe>
          </div>
        </div>
      </section>
      ...
      <Footer>
        <div className="wrap">
          <div className="footer-grid">
            ...
            <div className="footer-col">
              <h4>Contato</h4>
              <a href={WA} target="_blank" rel="noreferrer" className="footer-inline"><img src={images.icons.whatsapp} alt="" />{WA_DISPLAY}</a>
              <a href={IG_URL} target="_blank" rel="noreferrer" className="footer-inline"><img src={images.icons.instagram} alt="" />{company.contact.instagramHandle}</a>
              <p>{company.address.street} · {company.address.city}, {company.address.state}</p>
              <a href={waLink(company.messages.career)} target="_blank" rel="noreferrer" className="footer-careers"><BriefcaseBusiness aria-hidden="true" />Trabalhe conosco · Envie seu currículo</a>
            </div>
          </div>
          <div className="footer-bottom">
            <span>© {new Date().getFullYear()} {company.name}. {company.footer.copyright}</span>
            <span>{company.footer.signature} · {company.locationLabel}</span>
          </div>
        </div>
      </Footer>

      {/* hidden preload */}
      <img src={images.gallery.portrait1} alt="" style={{ display: "none" }} aria-hidden />
      <img src={images.gallery.portrait2} alt="" style={{ display: "none" }} aria-hidden />
      <img src={images.gallery.portraitLight} alt="" style={{ display: "none" }} aria-hidden />
      </div>
    </>
  );
}
